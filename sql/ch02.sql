...
-- 산술연산자
-- MySQL/ MariaDB - 블리언( boolean)을 정수형으로 처리한다.
-- 					True는 1, False는 0(단 null과의 비교는 null)
-- PostgreSQL(포스트그리-SQL): true/false텍스트형태의 불리언 값 반환
...
--  고객 테이블의 지역 필드가 ''(빈문자열)로 추가되엇다.
-- ''인지? '   '인지? null인지? 헷갈린다.
-- 지역 필드의 값을 모두'' -> null로 바꿔준다.
-- 값이 없을때는 명시적으로 null 값으로 채우는게 좋다.
UPDATE 고객
set 지역 = null -- where절이 없으면, 모든 열의 데에터가 적용된다.! 주의!
WHERE 지역 = '';
...
-- union은 중복된 행을 제거하고 출력(내부적으로 정렬 후 합친다.)


-- union 사용시 주의점
-- 1. 컬럼(필드) 갯수 일치
-- 2. 각 컬럼의 데이터 타입ㅂ(숫자, 문자, 날짜)
...
FROM 고객
WHERE  도시 like '%광역시'
and(고객번호 like '_C%'); -- 고객번호 2번쨰 문자가 C인 열을 찾는다.
...
-- 2.제품 테이블에서 단가가 5,000 원 이상 10,000원 이하인  
...
SELECT 제품번호, 제품명, 단가, 재고, 단가, 단가*재고 as '재고금액'
FROM 제품
order by 재고금액 desc
limit 10;
