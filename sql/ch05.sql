...
SELECT *
FROM 부서
JOIN 사원; -- ON조건이 없으면, CROSS JOIN으로 동작(Oracle에서는 오류발생)
...
FROM 부서
CROSS JOIN 사원; -- 40행(줄)
-- Non-ANSI 문법(MySQL, Oracle에서 사용 가능)
SELECT *
FROM 부서, 사원; -- 40개
...
-- 이너조인(INNER JOIN) : 두 테이블 상이의 공통된 값을 기준으올 결과를 반화
--  1. 등가조인(이퀴조인) :  = 등호로 비교한다.
--  2. 비등가조인(논이퀴조인) : 등호 외 비교연산자로 비교한다.
...
FROM 사원
INNER JOIN 부서
-- ON 사원.부서번호 = 부서.부서번호 on이 생략되면 cross join으로 나옴
WHERE 이름 = '이소미';
...
-- 아우터 조인(OUTR JOIN) : 조건(등가, 비등가)에 맞지 않는 행도 결과값으로 나옴.
SELECT * FROM 부서;

SELECT * 
FROM 사원; -- 총 사원은 10명, A4(홍보부)에 속한 사원이 없음.정수진(수습사원) 부서 없음

SELECT * 
FROM 사원 
WHERE 부서번호 = 'A4'; -- 홍보부에 속한 사원이 없음.
...
-- 사원 테이블에서 부서번호가 null인 행도 출력해보자.( 정수진은 교집합에 들어가 있지 않지만 출력하고 싶을때)
SELECT 부서명, 사원.*
FROM 사원
LEFT OUTER JOIN 부서
ON 사원.부서번호 = 부서.부서번호; 
...
-- 연습문제
-- 1. 세계무역 데이터베이스의 제품 테이블과 주문 세부 테일블을 조인하여 제품명별로 주문수량합과 주문금액합을 보이시오
DESC 제품;
DESC 주문세부;
...
SELECT  YEAR(주문일) AS 주문일, 제품.제품명, SUM(주문수량) AS 주문수량합
FROM 주문
JOIN 주문세부
ON 주문.주문번호 = 주문세부.주문번호
JOIN 제품
...
-- 결과만 보이시오.
SELECT 고객.도시, SUM(주문세부.주문수량*단가) AS 주문금액합
FROM 주문
INNER JOIN 주문세부
ON 주문.주문번호 = 주문세부.주문번호
INNER JOIN 고객
