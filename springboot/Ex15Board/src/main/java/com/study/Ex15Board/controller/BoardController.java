...
public class BoardController {
    ...
    //HTTP요청에서 보내주는 데이터 2가지
    //1. Parameter Key-Value Data : @RequestParm
    //2. Body Raw Data : @RequestBody @ModelAttribute(클래스나 Map에 매핑)
    //스프링(자바) 응답할때
    //1. @ResponseBody : HTTP Body Date(JASON/XML, 자바스크립트)
    //2. HTML파일 응답 : View Resolver에서 알아서 보낸다.
    ...
    public String writeAction(@ModelAttribute BoardSaveRequestDto dto){
        ...
        if(isFound) {
            ...
            //글쓰기(로그인, 회원가입)시 에 히스토리 백하면, 유저가 입력한 내용이 그대로 있다.
        }else {
