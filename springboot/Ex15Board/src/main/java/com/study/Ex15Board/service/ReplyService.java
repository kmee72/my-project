...
public class ReplyService {
    ...
    @Transactional
    public void update(Long replyIdx, String replyContent) {
        // 1. 댓글 조회
        Reply entity = replyRepository.findById(replyIdx)
                .orElseThrow(() -> new IllegalArgumentException("해당 댓글이 없습니다. id=" + replyIdx));

        // 2. 내용 수정 (이미 Reply 엔티티에 update 메서드가 정의되어 있음)
        entity.update(entity.getReplyName(), replyContent);
    }
}