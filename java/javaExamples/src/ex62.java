...
//  :JS배열.map() reduce() filter()

        //filter(조건필터링)
        List<Integer> nums = Arrays.asList(1,15,8,20,5,30);
        //10보다 큰 숫자만 리스트로 다시 만들기
        List<Integer> result = nums.stream()//체이닝메소드 (JS도 있음)
                .filter(n -> n > 10)
                .collect(Collectors.toList());
        System.out.println( result );

        //map (데이터 변형)
        List<String> words = Arrays.asList("appple", "banana", "cherry");
        //단어를 대문자로 변환하기
        ...
        List<String> tech = Arrays.asList("Java", "Spring", "Phython");
