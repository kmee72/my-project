...
// main함수 하나에 5000줄의 코드이면, 변수값(상태) 관린가 어렵다.
// public : 같은 폴더(패키지)에서, 다른 폴더의 클래스에서 접근이 가능.
// protected : 같은 폴더 + 상속관계 클래스에서 접근 가능
// default(생략) : 같은 폴더에서 접근 가능 (java에서는 기본이 default)
// private(제일중요)  : 같은 클래스 안에서 접근가능(캡슐화, 은닉)
//                  : 다른 클래스에서는 Getter/Setter 함수를 통해서만 접근 가능.
//                  : 변수나 함수를 최대한 노출하지 않는 것(값을 관리하기 위해서 접근제어)
// 클래스로 쪼개니...복잡하다.
class Hong{
    ...
    private  int engScore = 90; // private : 자기클래스에서만 접근 가능
    //Getter/Setter함수 제공
    public int getEngScore(){
        return  this.engScore;
    }
    public void setEngScore(int engScore){ // int.engScore는 지역변수(매개변수)
