//싱글톤(Singleton)
//  :프로그램 안에서 유일한 클래스 객체
//  :new 를 통해 객체를 여러번 생성하는 구조이기 때문에
//      같은 클래스의 객체가 여러개 있다면, 데이터의 유지관리가 어렵다.
//  :스프링 프레임워크에서는 싱글톤을 자동으로 유지/관리함.
//  :Bean (Spring 안에서 객체)이 죽으면 부활시켜주고, 중복안되게
//  :유일한 객체(싱글톤)이 필요한 이유
//      -유일한 정보를 저장/관리하기 위해
//  : 전역변수로 많이 쓰인다.
class FishBun{ //붕어빵
...
class  FishBunSingleton{
    int bunNo = 30;
    private static FishBunSingleton singleton; // 객체의 초기값은 null이다.
// getInstall메소드를 통해서 싱글톤 객체를 얻어온다.
    static FishBunSingleton getInstance(){ //JDK개발자 주로 사용한 함구.
        if(singleton == null){
...
// non-static(dynamic) 객체는 Heap 영역에 저장되고,
//GC(Garbage Collector)가 자동으로 메모리리에서 회수한다.
    }
}
